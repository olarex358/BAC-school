...
function StudentCertification() {
  ...
  // ✅ remove localhost
  const [certificationResults, , loadingCertResults] = useLocalStorage(
  ...
  const calculateGradeAndQualification = (total) => {
    ...
    const scaledScore = (Number(total || 0) / 100) * 9;
  ...
  return (
    <div className="content-section">
      ...
      <p>
        Welcome, {loggedInStudent.firstName} {loggedInStudent.lastName}! Here are
              ...
              {studentSpecificResults.map((res) => {
                ...
                return (
                  <tr key={res._id || res.id}>
                    ...
                    <td>{res.pracScore}</td>
                    <td>
                      <strong>{res.totalScore}</strong>
                    </td>
                    <td style={{ color: qualified ? "green" : "red" }}>
                      <strong>{grade}</strong>
                    </td>
                    <td style={{ color: qualified ? "green" : "red" }}>
                      {qualified ? "Yes" : "No"}
      ...
      <p style={{ marginTop: "20px" }}>
        For official transcripts and verification of your certification, please contact
